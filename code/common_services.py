...
def get_class_common_prefixed(class_obj, prefix_len=7, prefix_to_search=None) -> List[str]:
    """
    
    :param class_obj: 
    :param prefix_len: used only if method_prefix is None 
    :param prefix_to_search: 
    :return: 
    """
    prefix_count = dict()
    for i in class_obj.__dict__.keys():
        i_prefixed = i[:prefix_len]
        if i_prefixed not in prefix_count.keys():
            prefix_count[i_prefixed] = 1
        else:
            prefix_count[i_prefixed] += 1
    if prefix_to_search is None:
        # REFER: https://www.geeksforgeeks.org/python-get-key-with-maximum-value-in-dictionary/
        prefix_to_search = max(prefix_count, key=prefix_count.get)
    return [str(i) for i in class_obj.__dict__.keys()
            if str(i).startswith(prefix_to_search)]
